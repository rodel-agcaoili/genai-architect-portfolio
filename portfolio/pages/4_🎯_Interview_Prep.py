...
try:
    from utils.interview_engine import (
        ...
        generate_interview_report, save_session, get_progress_summary,
    )
...
if st.session_state.iv_phase == "setup":
    ...
    if progress["total_sessions"] > 0:
        ...
        for s in progress["sessions"][:5]:
            ts = s["timestamp"][:10]
            grade_col = _grade_color(s.get("overall_grade", "C"))
            st.markdown(
                f'<div style="background: rgba(30,30,60,0.6); border-left: 3px solid {grade_col}; '
                f'padding: 0.8rem 1rem; border-radius: 6px; margin-bottom: 0.5rem;">'
                f'<strong style="color: #e0e0ff;">{s.get("jd_title", "Untitled")}</strong><br>'
                f'<span style="color: #a0a0d0; font-size: 0.8rem;">{ts} · '
                f'{s.get("role_level", "")} · Score: {s.get("overall_score", 0):.0f} · '
                f'{s.get("readiness", "")}</span></div>',
                unsafe_allow_html=True,
            )
...
# ========================== PHASE 2: INTERVIEW ==========================
    ...
    if current_q < total_q:
        ...
        with mic_col:
            mic_clicked = st.button("🎤 Speak", key=f"mic_{current_q}", use_container_width=True, type="primary")

        if mic_clicked:
            st.components.v1.html("""
            <div id="mic-status" style="background: rgba(15,12,41,0.9); border: 1px solid #7c83ff; border-radius: 8px; padding: 1rem; margin: 0.5rem 0; font-family: sans-serif; color: #e0e0ff; font-size: 0.9rem;">
                <div id="mic-text">🎤 Requesting microphone...</div>
                <div id="mic-hint" style="color: #a0a0d0; font-size: 0.75rem; margin-top: 0.3rem;">Click "Allow" if prompted.</div>
            </div>
            <script>
                const statusEl = document.getElementById('mic-text');
                const hintEl = document.getElementById('mic-hint');
                const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
                if (!SR) { statusEl.innerHTML = '❌ Use Chrome or Edge for speech.'; } else {
                    const r = new SR(); r.lang='en-US'; r.interimResults=true;
                    r.onstart = () => { statusEl.innerHTML='🔴 Listening...'; hintEl.innerHTML='Speak your answer now.'; };
                    r.onresult = (e) => {
                        const t=e.results[0][0].transcript; const f=e.results[0].isFinal;
                        if(f){statusEl.innerHTML='✅ '+t; hintEl.innerHTML='Paste this into the text box below, then submit.';
                        } else { statusEl.innerHTML='🎤 '+t+'...'; }
                    };
                    r.onerror = (e) => { statusEl.innerHTML='❌ '+e.error; };
                    r.onend = () => { if(!statusEl.innerHTML.includes('✅')&&!statusEl.innerHTML.includes('❌')) statusEl.innerHTML='⏹️ No speech detected.'; };
                    r.start();
                }
            </script>
            """, height=80)
